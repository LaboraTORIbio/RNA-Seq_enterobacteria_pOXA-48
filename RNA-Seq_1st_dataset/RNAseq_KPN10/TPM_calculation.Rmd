...
```{r, eval=TRUE}
plasfreenames <- c("PF_KPN10.1", "PF_KPN10.2")
...
df[df == "cds-pgaptmp_005541"] <- "blaOXA-48"

# Only chromosomal and pOXA-48 rows are selected
df <- subset(df, Chr %in% c("1", "1;1", "4"))
df$Chr[df$Chr %in% c("1", "1;1")] <- "Chromosome"
df$Chr[df$Chr %in% c("4")] <- "pOXA-48"
...
```{r, eval=TRUE}
cor(tpm_all[,1:2], method="spearman")
```

Kruskal-Wallis test followed by Dunn's test to assess differences between replicates.

```{r, eval=TRUE}
wilcox.test(dfmelted$TPM, g = dfmelted$Replicate)
```

### Calculating mean TPM values

```{r, eval=TRUE}
df$median_tpm <- apply(df[, 2:3], 1, median) # change replicate columns if necessary
head(df)
tail(df)
write.table(df, file="TPM_KPN10_chr_pOXA48.tsv", sep="\t", row.names=FALSE)
```
