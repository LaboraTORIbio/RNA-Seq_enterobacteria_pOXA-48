...
df[df == "cds-pgaptmp_003256"] <- "lysR"
df[df == "cds-pgaptmp_003257"] <- "pfp"
df[df == "cds-pgaptmp_003258"] <- "ifp"
...
```{r, eval=TRUE}
df$median_tpm_wop <- apply(df[, 2:3], 1, median) # change replicate columns if necessary
df$median_tpm_wpl <- apply(df[, 4:5], 1, median) # change replicate columns if necessary
head(df)
tail(df)
write.table(df, file="TPM_KPN10_operon_annot.tsv", sep="\t", row.names=FALSE)
```
