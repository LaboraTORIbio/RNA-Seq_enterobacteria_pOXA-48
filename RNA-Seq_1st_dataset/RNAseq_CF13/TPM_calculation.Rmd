...
```{r, eval=TRUE}
plasfreenames <- c("CF13c1.1", "CF13c1.2")
...
df[df == "cds-pgaptmp_004900"] <- "blaOXA-48"

# Only chromosomal and pOXA-48 rows are selected
df <- subset(df, Chr %in% c("1", "1;1", "2"))
df$Chr[df$Chr %in% c("1", "1;1")] <- "Chromosome"
df$Chr[df$Chr %in% c("2")] <- "pOXA-48"
...
head(df)
tail(df)
write.table(df, file="TPM_CF13_chr_pOXA48.tsv", sep="\t", row.names=FALSE)
```
