...
df[df == "cds-pgaptmp_001596"] <- "lysR"
df[df == "cds-pgaptmp_001595"] <- "pfp"
df[df == "cds-pgaptmp_001594"] <- "ifp"
...
df$median_tpm_wop <- apply(df[, 5:6], 1, median) # change replicate columns if necessary
head(df)
tail(df)
write.table(df, file="TPM_CF13_operon_annot.tsv", sep="\t", row.names=FALSE)
```
