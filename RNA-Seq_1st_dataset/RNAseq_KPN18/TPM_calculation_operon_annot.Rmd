...
df[df == "cds-pgaptmp_003337"] <- "lysR"
df[df == "cds-pgaptmp_003338"] <- "pfp"
df[df == "cds-pgaptmp_003339"] <- "ifp"
...
```{r, eval=TRUE}
df$median_tpm_wop <- apply(df[, 2:4], 1, median) # change replicate columns if necessary
df$median_tpm_wpl <- apply(df[, 5:7], 1, median) # change replicate columns if necessary
head(df)
tail(df)
write.table(df, file="TPM_KPN18_operon_annot.tsv", sep="\t", row.names=FALSE)
```
