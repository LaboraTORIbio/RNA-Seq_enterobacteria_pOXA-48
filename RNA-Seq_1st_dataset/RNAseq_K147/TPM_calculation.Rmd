...
```{r, eval=TRUE}
plasfreenames <- c("K147c1.1", "K147c1.2")
...
df[df == "cds-pgaptmp_005308"] <- "blaOXA-48"
...
```{r, eval=TRUE}
cor(tpm_all[,1:4], method="spearman")
```
...
```{r, eval=TRUE}
df$median_tpm <- apply(df[, 2:5], 1, median) # change replicate columns if necessary
head(df)
tail(df)
write.table(df, file="TPM_K147_chr_pOXA48.tsv", sep="\t", row.names=FALSE)
```
