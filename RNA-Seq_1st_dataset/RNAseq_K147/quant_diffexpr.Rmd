...
```{r, eval=TRUE, class.source="bg-success"}
resfilt <- results(dds, alpha = 0.05)
table(resfilt$padj < 0.05)
resfilt
summary(resfilt)
```
...
resFilt <- subset(resfilt, padj < 0.05)
## Merge with normalized count data and annotation
resdatafilt <- merge(as.data.frame(resFilt), as.data.frame(counts(dds, normalized=TRUE)), by="row.names", sort=FALSE)
names(resdatafilt)[1] <- "GeneID"
resdatafilt <- merge(x=as.data.frame(resdatafilt), saftable, by="GeneID")
head(resdatafilt)
## Order by log2FC
resdatafilt <- resdatafilt[order(resdatafilt$log2FoldChange, decreasing = TRUE), ]
## Write results
write.table(resdatafilt, file="DE_results_filtered_padj.tsv", sep="\t", row.names=FALSE)
```
