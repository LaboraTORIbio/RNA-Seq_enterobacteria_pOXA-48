...
df[df == "cds-pgaptmp_003232"] <- "lysR"
df[df == "cds-pgaptmp_003233"] <- "pfp"
df[df == "cds-pgaptmp_003234"] <- "ifp"
...
df$median_tpm_wpl <- apply(df[, 4:6], 1, median) # change replicate columns if necessary
head(df)
tail(df)
write.table(df, file="TPM_KPN15_operon_annot.tsv", sep="\t", row.names=FALSE)
```
